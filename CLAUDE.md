...
- **`src/demo/`** — a mock Tauri backend (`mockIPC`) plus the example data it
  serves, swapped in for the real one by angular.json's `demo` build
  configuration (`src/main.ts` -> `src/main.demo.ts`). It exists so the whole
...
- Never hand-edit version numbers; six files must agree, so use
  ...
  against.
- Never hand-edit or hand-capture the README's screenshots either. They are
  generated by `npm run screenshots` from the example data in
  `src/demo/demo-data.ts`; output is byte-identical between runs, so a
  changed PNG means the UI changed. See `.claude/skills/screenshots/`.
